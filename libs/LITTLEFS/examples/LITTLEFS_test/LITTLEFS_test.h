...
void listDir(fs::FS &fs, const char * dirname, uint8_t levels){
    ...
    char buffer[128]; // assuming max pathname equals 256
    ...
    while(file){
        if(file.isDirectory()){
            ...
            if(levels){
                snprintf(buffer, sizeof(buffer), "%s%s", dirname, "/");         // if dirname is "/", you get "//" in the lead instead of "/", but that still appears to work fine.
                snprintf(buffer, sizeof(buffer), "%s%s", buffer, file.name());
