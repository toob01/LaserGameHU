...
uint8_t shiftInSlow(uint8_t dataPin, uint8_t clockPin/*, uint8_t bitOrder*/) 
{
    ...
    for(i = 0; i < 8; ++i) {
		ard::digitalWrite(clockPin, ard::high);
		ard::delayMicroseconds(1);
        //if(bitOrder == LSBFIRST)
        //    value |= ard::digitalRead(dataPin) << i;
        //else
        value |= ard::digitalRead(dataPin) << (7 - i);
		ard::digitalWrite(clockPin, ard::low);
		ard::delayMicroseconds(1);
    }
    return value;
}
...
void HX711::begin(uint8_t dout, uint8_t pd_sck, uint8_t gain) {
	ESP_LOGI("mav", "hx711 begin start"); vTaskDelay(1);
	PD_SCK = pd_sck;
	DOUT   = dout;

	ard::pinMode((int)PD_SCK, gpio_mode_t::GPIO_MODE_OUTPUT);
	ard::pinMode((int)DOUT, gpio_mode_t::GPIO_MODE_INPUT/*DOUT_MODE*/);  // something goes wrong here.

	set_gain(gain);
	ESP_LOGI("mav", "hx711 begin end"); vTaskDelay(1);
}

bool HX711::is_ready() {
	return ard::digitalRead(DOUT) == ard::low;
}
...
//	#endif
	...
	// Scope for TaskCriticalSection:
	{
		crt::TaskCriticalSection tcs;
		...
		for (unsigned int i = 0; i < GAIN; i++) {
			ard::digitalWrite(PD_SCK, ard::high);
	//		#if ARCH_ESPRESSIF
			ard::delayMicroseconds(1);
	//		#endif
			ard::digitalWrite(PD_SCK, ard::low);
	//		#if ARCH_ESPRESSIF
			ard::delayMicroseconds(1);
	//		#endif
		}
...
void HX711::wait_ready(unsigned long delay_ms) {
	...
	// Wait for the chip to become ready. That takes about 100ms per measurement, for high-precision measurements.
	...
	//crt::logger.logText("wait_ready took ms:");
	//crt::logger.logUint32((uint32_t)(after - before));
}
...
bool HX711::wait_ready_timeout(unsigned long timeout, unsigned long delay_ms) {
	...
	unsigned long millisStarted = ard::millis();
	while (ard::millis() - millisStarted < timeout) {
		if (is_ready()) {
			return true;
		}
		vTaskDelay(delay_ms);
...
long HX711::read_average(uint8_t times) {
	...
	for (uint8_t i = 0; i < times; i++) {
		...
		sum += read();
		//uint64_t after = ard::millis();
		//crt::logger.logText("read took ms:");
		//crt::logger.logUint32((uint32_t)(after - before));
		// Probably will do no harm on AVR but will feed the Watchdog Timer (WDT) on ESP.
		// https://github.com/bogde/HX711/issues/73
		//before = ard::millis();
		vTaskDelay(1);
		//after = ard::millis();
		//crt::logger.logText("vTaskDelay(1) took ms:");
		//crt::logger.logUint32((uint32_t)(after - before));
		//taskYIELD(); does yield, but not feed the watchdog timer.
		//vTaskDelay(0); does yield, but not feed the watchdog timer.
	}
	return sum / times;
}
