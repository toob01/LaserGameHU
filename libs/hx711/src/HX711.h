#pragma once
//#include "Arduino.h"
#include <crt_FreeRTOS.h> // for types like uint8_t
class HX711
{
	private:
	...
	public:
		...
		void wait_ready(unsigned long delay_ms = 1); // don't make the delay 0, because it would generate a long busy wait.
		bool wait_ready_retry(int retries = 3, unsigned long delay_ms = 1);
		bool wait_ready_timeout(unsigned long timeout = 1000, unsigned long delay_ms = 1);
